...
public abstract class DenterHelper {
    ...
    public Token nextToken() {
        ...
        final Token r;
        if (t.getType() == pythonLexer.LBRACK || t.getType() == pythonLexer.LPAREN || t.getType() == pythonLexer.LBRACE) {
            bracketDepth++;
        }
        if (t.getType() == pythonLexer.RBRACK || t.getType() == pythonLexer.RPAREN || t.getType() == pythonLexer.RBRACE) {
            bracketDepth--;
        }
    ...
    private Token handleNewlineToken(Token t) {
        ...
        if (bracketDepth > 0) {
            // Inside brackets: skip newlines entirely, return next real token
            Token next = pullToken();
            while (next.getType() == nlToken) {
                next = pullToken();
            }
            return next;
        }
        ...
        // Enforce 4-space indentation (Python standard)
        if (indent > 0 && indent % 4 != 0) {
            throw new RuntimeException(
                    "IndentationError at line " + nextNext.getLine()
                    + ": indentation must be multiple of 4 spaces, got " + indent + " spaces"
            );
        }
     ...
     * necessary.
     *
     * @param targetIndent the "size" of the indentation (number of spaces) by
     * the end
