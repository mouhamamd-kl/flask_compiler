...
| **ProjectGenerator.java** | `src/antlr/generator/ProjectGenerator.java` | منسّق المرحلة الثانية - توليد HTML من Flask |
| **PythonEvaluator.java** | `src/antlr/generator/python/PythonEvaluator.java` | تنفيذ كود إعداد البيانات والتقاط `render_template` |
| **JinjaRenderer.java** | `src/antlr/generator/jinja/JinjaRenderer.java` | عرض قالب Jinja2 إلى HTML |
| **FlaskProjectAnalyzer.java** | `src/antlr/semantic/flask/FlaskProjectAnalyzer.java` | فحص دلالي على مستوى المشروع |
| **PythonSemanticAnalyzer.java** | `src/antlr/semantic/python/PythonSemanticAnalyzer.java` | فحص دلالي لكود Python |
| **JinjaSemanticAnalyzer.java** | `src/antlr/semantic/jinja/JinjaSemanticAnalyzer.java` | فحص دلالي للقوالب |
| **RTValue.java** | `src/antlr/runtime/values/RTValue.java` | أساس نظام قيم الـ runtime |
| **LiveServer.java** | `src/antlr/serve/LiveServer.java` | الخادم المباشر |
| **ReloadHub.java** | `src/antlr/serve/ReloadHub.java` | WebSocket لإعادة تحميل المتصفح |
...
## المرحلة الثانية: توليد الكود (Code Generation)

المترجم إلو **مرحلتين**. المرحلة الأولى (اللي شرحناها فوق) بترجّم ملف Python أو قالب Jinja2 لحالو وتُظهر الـ AST وجدول الرموز. أما **المرحلة الثانية** فبتحوّل **مشروع ويب كامل مكتوب بـ Flask + Jinja2** إلى **صفحات HTML ثابتة** جاهزة للنشر — مع فحص دلالي متكامل قبل التوليد.

### شو يعني "توليد كود"؟

الدخلة هي مشروع Flask عادي فيه:
- `app.py` - الكود الخلفي (البيانات + الـ routes)
- `templates/` - قوالب Jinja2 مع HTML و CSS
- `style.css` و `script.js` (اختياري)

والخرجة هي صفحات HTML **ثابتة** - يعني القوالب انربطت بالبيانات الحقيقية من Python، فصارت صفحات جاهزة بدون ما تحتاج تشغيل السيرفر.

**مثال - قبل التوليد (قالب Jinja2):**
```jinja2
...
**بعد التوليد (HTML ثابت):**
```html
<h3>Laptop - $999.99</h3>
<h3>Phone - $699.99</h3>
```

### المخطّط العام للمرحلة الثانية

```text
            app.py (ملف Flask)
                 ↓
        Python Parser  →  ast_python.json
                 ↓
      Semantic Analysis  →  semantic_report.txt
                 ↓
      Generator / Evaluator (تنفيذ كود إعداد البيانات)
                 ↓
      Context Data (المتغيرات والقيم المولدة)
                 ↓
      render_template() requests  ← تدخل للقوالب
                 ↓
           قالب Jinja2
                 ↓
          Jinja Parser  →  ast_jinja.json
                 ↓
       Variable Substitution {{ name }} {{ price }}
                 ↓
              HTML ثابت  →  output/*.html
```

**باختصار:**
- **Python** → لتحضير البيانات فقط → تمريرها للقالب
- **Jinja2** → لعرض البيانات → HTML

---

### مراحل التوليد الخمسة

الكل بتنفذها كلاس `ProjectGenerator.java`، وبكل مرحلة بتتسجّل تفاصيلها بـ `generation_log.txt`.

#### المرحلة A: تحليل app.py

بنقرأ `app.py`، نمرّرو على نفس خط أنابيب Python (lex → parse → AST)، وبنعمل **التحليل الدلالي** للـ Flask:
```java
...
**الخرجات:**
- `compiler_output/ast_python.json` - AST لملف Python بصيغة JSON
- `compiler_output/semantic_report.txt` - تقرير الدلالة

لو في أخطاء دلالية، بتتوقف عملية التوليد فوراً.

#### المرحلة B: تنفيذ كود إعداد البيانات

هون بننفّذ الجزء اللي **بجهّز البيانات** من كود Python (قوائم المنتجات، الحسابات…) بدون ما نشتغل حقيقياً على الويب. الـ `PythonEvaluator` بيمشي على الـ AST وبينتج سجل من قيم runtime، و**بيمسك كل استدعاء `render_template(...)`** ويحولّو لطلب `RTRenderRequest` فيه اسم القالب + سياق المتغيرات.
...
هي الاستدعائية بتتعامل كطلب توليد صفحة، بـ: القالب `products.html` والمتغير `products`. فبعدين بنجيب القالب وبنربطو بالبيانات.

#### المرحلة C: تحليل القوالب ودلالاتها

بنقرأ كل قالب مرجعي من `templates/`، ونبنيلو AST، ونحفظ كل القوالب بسجل مشترك (عشان دعم `{% include %}` و `{% extends %}`).

بعدها **التحليل الدلالي على مستوى المشروع** - `FlaskProjectAnalyzer` بيفحص:
- **شو المتغيرات اللي القالب عم يستخدمها** `{{ name }}`
- **شو المتغيرات اللي `render_template` عم يوفرها**
- لو في متغير عم يستخدم بالقالب وما في حدا عم يوفرّو → خطأ **`MISSING_TEMPLATE_VARIABLE`**

```text
products.html:3:3 [MISSING_TEMPLATE_VARIABLE] - Semantic error:
template variable 'products' is not provided by render_template()
and is not defined in the template
```

هاي ميزة قوية: بتضمن إنو **ما في قالب بيمرّي متغير غير موجود** - بيكتشف المشكلة قبل التوليد.

- `compiler_output/ast_jinja.json` - AST لكل القوالب

#### المرحلة D: العرض (Rendering)

`JinjaRenderer` بيمشي على AST القالب، وبستبدل `{{ expression }}` بقيمتها الحقيقية من السياق، وبينفّذ `{% for %}` و `{% if %}` و `{% include %}`… لحتى ينتج HTML نهائي.

```java
String html = renderer.render(req.template, req.request.getTemplateName(), context);
```

بعدها بكتب ملف HTML بـ `output/` بالاسم المناسب:
- صفحة ثابتة → `output/products.html`
- صفحة ديناميكية (route مع معامل مثل `/product/5`) → `output/product_5.html`

وبعدين **`StaticLinkRewriter`** بيصلّح الروابط: أي `href` أو `url_for` بيحوّلو لمسار الملف الثابت الصحيح.

#### المرحلة E: الملفات المرافقة

الملفات اللي **مش جزء من التحليل** (بيتم نسخها كما هي بدون أي معالجة):
- `app.py` (نسخة للتشغيل)
- `style.css` و `script.js` (لو موجودة)

### المخرجات النهائية

```text
output/                          ← الصفحات المولّدة (HTML ثابت)
├── products.html
├── product_1.html
├── product_2.html
├── product_3.html
└── app.py, style.css, script.js  ← منسوخة كما هي

compiler_output/                 ← مخرجات التحليل والتوليد
├── ast_python.json
├── ast_jinja.json
├── semantic_report.txt
└── generation_log.txt
...
### التوليد المتزايد (Incremental Generation)

بوضع `--generate --watch`، بيمشي على تغييرات الملفات ويعيد التوليد **بذكاء**:
- لو تغيّر `app.py` → بيعيد كل المراحل
- لو تغيّر قالب → بيعيد تحليل وعرض **الصفحات المتأثرة فقط**

```java
if (changedAbs.equals(appFileAbs)) {
    regenerateFromAppPy(startMs);      // كل المراحل
} else {
    regenerateFromTemplate(baseName, startMs);  // القالب + العرض
}
```
...
عشان ننفّذ كود Python (المرحلة B) ونعرض القوالب (المرحلة D)، بنحتاج تمثيل للقيم أثناء التشغيل. هون بيجي دور **`RTValue`** - نظام قيم موحد بيدعم كل أنواع Python:
...
├── RTString     - نصوص
├── RTList       - قوائم
├── RTDict       - قاموس (key → value)
├── RTFunction   - دوال (لتنفيذ التعريفات والاستدعاءات)
├── RTNone       - none
└── RTUndefined  - قيمة غير معرفة
```

**ليش هاد النظام ضروري؟** لأنو المترجم بنفّذ "سكربت" إعداد البيانات (اللي بجهّز قوائم المنتجات مثلاً) لحتى يقدر يجيب القيم الحقيقية ويمرّرا للقالب — مش بس يعرض الشجرة.

**مثال بسيط:**
```java
// products = [...]  →  RTList يحتوي RTDict لكل منتج
RTList products = new RTList();
RTDict laptop = new RTDict();
laptop.set("name", new RTString("Laptop"));
laptop.set("price", new RTFloat(999.99));
products.add(laptop);
```

- الحسابات (`PythonOperators`, `ValueOps`) بتشتغل على هالقيم
- الاستدعاءات بتنفّذ من `FunctionInvoker`
- الدوال المدمجة (`print`, `max`, `range`, `float`…) موجودة بـ `Builtins`

---

## التحليل الدلالي متعدد المستويات

المشروع إلو **ثلاثة مستويات** للفحص الدلالي، وكل مستوى بيكشف نوع معين من الأخطاء:

### 1. مستوى Python - `PythonSemanticAnalyzer`

بيفحص كود Python بدون ما ينفّذو (نفس فئة أخطاء CPython):

| الكود | المعنى | مثال |
|-------|--------|------|
| `UNDEFINED_VARIABLE` | متغير مستخدم قبل تعريفه (NameError) | `print(x)` بدون `x = ...` |
| `TYPE_ERROR` | استدعاء/فهرسة شيء مش قابلو | `products()` مع `products` قائمة |
| `TYPE_MISMATCH` | أنواع معاملات غير متوافقة | `'a' - 5` أو قائمة مفهرسة بنص |
| `SCOPE_ERROR` | `break`/`return` بغير مكانو | `break` خارج حلقة |

**ملاحظة ذكية:** أي شيء بيعتمد على قيم وقت التشغيل بيتبقّى `ANY` - يعني المحلل **بيفلّر بس الأخطاء اللي Python بترفضهن دايماً**، وما بيخمّن.

### 2. مستوى Jinja - `JinjaSemanticAnalyzer`

بيمشي على AST القالب ويتتبع المتغيرات والنطاقات داخل القالب (ماكرو، for، block…).

### 3. مستوى المشروع - `FlaskProjectAnalyzer`

بينظّم `render_template(...)` مع قوالبها ويكتشف متغيرات القالب غير المؤمّنة (`MISSING_TEMPLATE_VARIABLE`) - شرحناه بالمرحلة C.

### أمثلة على أخطاء دلالية مكتشفة

بالمجلد `tests/error-demos/` في أمثلة جاهزة:

| المجلد | الخطأ المكتشف |
|--------|----------------|
| `undefined-variable/` | متغير غير معرف |
| `type-error/` | استدعاء شيء مش قابل للاستدعاء |
| `type-mismatch/` | تعارض أنواع |
| `scope-error/` | `break`/`return` خارج مكانها |
| `missing-flask-variable/` | متغير قالب غير مؤمّن من `render_template` |

---

## الخادم المباشر (Live Server)

الميزة الأخيرة هي وضع **`serve`** - بيقلّد `flask --app app run` بمعنى حقيقي: بيفتح سيرفر HTTP يعرض تطبيق Flask فعلياً على المتصفح، مع **إعادة تحميل تلقائية** عند أي تعديل.

### كيف يشتغل؟

```bash
java Main serve            # سيرفر على منفذ عشوائي (tests/flask/)
java Main serve 9000 tests/flask   # سيرفر على :9000 لمجلد محدد
```

- بيقرأ `app.py` ويبنيه كتطبيق Flask حي (`LiveApp`)
- `LiveServer` بيستمع لطلبات HTTP ويخدم الصفحات بالسيرفر الحقيقي
- `ReloadHub` (WebSocket) بيمكّن المتصفح من **تحديث نفسه تلقائياً** لما يتغير الملف

### إعادة التحميل التلقائي

عند أي تعديل بملف:
- تغيّر `app.py` → بيقراه من جديد ويعيد ضبط الحالة (`state reset`) + `server.reload()`
- تغيّر قالب → بيحدّث القوالب (`reloadTemplates()`) + `server.reload()`

```bash
🔄 Change detected: app.py
✓ app.py reloaded (state reset).
✓ templates reloaded.
```

المتصفح بيرجع يتحدث لحالو عبر WebSocket بدون أي refresh يدوي — نفس تجربة Flask `debug=True` بالضبط.
...
4. **تشغيل Main.java:**
   ...
   **① أوضاع المرحلة الأولى (تحليل وعرض):**
   ```bash
   # تشغيل كل الاختبارات
   java Main
   ...
   # وضع المراقبة - إعادة الترجمة عند أي تعديل
   java Main --watch
   java Main --watch tests/flask
   ```

   **② توليد الكود - تحويل مشروع Flask إلى HTML ثابت:**
   ```bash
   # توليد كل صفحات HTML من مشروع Flask
   java Main --generate tests/flask

   # توليد + مراقبة (إعادة توليد تلقائية عند التعديل)
   java Main --generate --watch tests/flask
   ```

   **③ الخادم المباشر (مثل `flask --app app run`):**
   ```bash
   # سيرفر على منفذ عشوائي
   java Main serve

   # سيرفر على منفذ محدد لمجلد معين
   java Main serve 9000 tests/flask
   ```

### شرح خيارات سطر الأوامر

| الخيار | الاختصار | الوظيفة |
|--------|----------|---------|
| `--generate [dir]` | `-g` | المرحلة الثانية: توليد HTML من مشروع Flask |
| `--watch [dir]` | `-W` | مراقبة الملفات وإعادة الترجمة/التوليد تلقائياً |
| `--hide-whitespace` | `-w` | إخفاء عقد النص البيضاء بالعرض |
| `serve [port] [dir]` | `-S` | خادم مباشر مع تحديث تلقائي بالمتصفح |
...
| `tests/semantic/valid.py` | كود Python صحيح يدعم الفحص الدلالي |
| `tests/error-demos/undefined-variable/app.py` | مثال خطأ: متغير غير معرف |
| `tests/error-demos/type-error/app.py` | مثال خطأ: استدعاء شيء غير قابل للاستدعاء |
| `tests/error-demos/type-mismatch/app.py` | مثال خطأ: تعارض أنواع |
| `tests/error-demos/scope-error/app.py` | مثال خطأ: break/return خارج مكانها |
| `tests/error-demos/missing-flask-variable/app.py` | مثال خطأ: متغير قالب غير مؤمّن |

---

## الخلاصة

هاد المشروع بيوضح بشكل عملي كامل كيف بنبني مترجم احترافي من الصفر باستخدام ANTLR. المشروع بيشتغل على **مرحلتين**:
...
### المرحلة الثانية - توليد الكود
✅ **من Flask إلى HTML ثابت** - تحويل مشروع ويب كامل لصفحات جاهزة
✅ **تنفيذ إعداد البيانات** - تنفيذ كود Python وتمرير قيمه للقوالب
✅ **فحص على مستوى المشروع** - كشف متغيرات القوالب غير المؤمّنة
✅ **التوليد المتزايد** - إعادة توليد ذكية عند التعديل
✅ **خادم مباشر** - تشغيل التطبيق مع تحديث تلقائي بالمتصفح عبر WebSocket
